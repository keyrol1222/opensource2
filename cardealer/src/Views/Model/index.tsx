...
import { useApiData } from '../../Services/actions';
    ...
    type item = { state: number, name: string, brand: number, id: number}
function Model() {
    const [showModal, setShowModal] = useState(false);
    const [brands, setBrands] = useState([]);
    const [selected, setSelected] = useState<item>({ state: 0, name: '', brand: 0, id: 0 });
...
/>
        ...
        <TERipple rippleColor="light" className="w-full mt-2">
          <button
            type="button"
            disabled={selected.state === 0 || selected.name === '' || selected.brand === 0}
            onClick={() => {
              if(selected.state === 0 || selected.name === '') {
                ...
                if(mode === 'create'){
                  ...
                  if(mode === 'edit'){
                    ...
                    useApi.putData(selected.brand, data, ()=>{
                      setSelected({ state: 0, name: '', brand: 0, id: 0});
                      setShowModal(false);
                      useApi.callApi();
                    });
    ...
    <header>
            <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
              <h1 className="text-3xl font-bold leading-tight tracking-tight text-gray-900">Modelos</h1>
            </div>
          </header>
    <div className='m-2'>
        <button
